  ...
  $scope.test_programs = [
    {
      ...
      description: "This program uses the Master Programmer to orchestrate the other units. First it tells the constant transmitter to send X to Accumulator 1. Then it tells the constant transmitter to send Y to Accuulator 2. It then tells the Accumulators to subtract the numbers and triple the difference. You should see the result in Accumulator 2 after 13 'Addition Times.'"
    },
    {
      id: 1,
      name: "Add Two Constants",
      program: TestAddTwoConstantsOnce,
      description: "This program uses the constant transmitter and both accumulators to simply add two numbers that are initially stored in the constant transmitter. Rather than relying on the Master Programmer, each operation is responsible for initiating the next operation in the chain of operations."
    },
    {
      id: 2,
      name: "Transfer a Negative Number",
      program: TestAccToAccNegNumTransfer,
      description: "This is a simple program to test the sending of a negative number across the digit tray (data bus)."
    },
  ];
